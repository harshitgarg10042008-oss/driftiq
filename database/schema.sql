...
DO $$ BEGIN
ALTER TABLE files
ALTER COLUMN telegram_file_id TYPE TEXT;
EXCEPTION
WHEN others THEN NULL;
END $$;
...
END;
$$ LANGUAGE plpgsql;
DO $$ BEGIN CREATE TRIGGER trigger_users_updated_at BEFORE
UPDATE ON users FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
EXCEPTION
WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN CREATE TRIGGER trigger_files_updated_at BEFORE
UPDATE ON files FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
EXCEPTION
WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN CREATE TRIGGER trigger_folders_updated_at BEFORE
UPDATE ON folders FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
EXCEPTION
WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN CREATE TRIGGER trigger_shares_updated_at BEFORE
UPDATE ON shares FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
EXCEPTION
WHEN duplicate_object THEN NULL;
END $$;
-- STEP 4: Indexes
...
CREATE OR REPLACE FUNCTION increment_download_count(share_id UUID) RETURNS void AS $$ BEGIN
UPDATE shares
SET download_count = COALESCE(download_count, 0) + 1
WHERE id = share_id;
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION get_total_storage_used() RETURNS BIGINT AS $$
DECLARE total BIGINT;
BEGIN
SELECT SUM(storage_used) INTO total
FROM users;
